...
class UpdateInformationSheetRequest extends FormRequest
{
    ...
    protected function prepareForValidation(): void
    {
        ...
        if ($this->hasFile('director_signature') && ! $this->file('director_signature')->isValid()) {
            $this->files->remove('director_signature');
        }
    }
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator) {
            ...
            $data = collect($validator->getData())
                ->except(['_token', '_method', 'founder_signature', 'director_signature'])
                ->all();
    ...
    public function rules(): array
    {
        return [
            ...
            // Startup-level fields (target_market/problem_statement/solution_offered
            // have no edit UI anywhere else in the app yet).
            ...
            'portfolio_manager' => ['nullable', 'string', 'max:150'],
            'cohort_no' => ['nullable', 'string', 'max:20'],
            'endorsed_by' => ['nullable', 'string', 'max:150'],
            'endorsement_date' => ['nullable', 'date'],
            'director_approval_date' => ['nullable', 'date'],
            'director_signature' => ['nullable', 'image', 'max:20480'],
        ];
    }
}
