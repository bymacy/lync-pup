<x-layouts.founder title="Meeting">
    ...
    <div class="flex items-center gap-2 mb-6">
        <span class="icon-mask w-10 h-10 text-[#6D0D23]"
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="flex bg-white border rounded-2xl overflow-hidden">
            {{-- Left band --}}
            <div class="w-40 flex-shrink-0 bg-[#6C0E24] text-white flex flex-col items-center justify-center text-center px-4 py-6">
            ...
            {{-- Action panel --}}
            <div class="w-44 flex-shrink-0 flex items-center justify-center px-4 py-5">
                @if ($meeting['type'] === 'mentorship')
                @if ($meeting['can_join'] && $meeting['meeting_link'])
                <a href="{{ $meeting['meeting_link'] }}" target="_blank" rel="noopener"
                    class="w-full text-center bg-gradient-to-r from-[#6D0D23] to-[#11386A] text-white text-sm font-medium rounded-lg py-2.5">
                    Join Meeting
                </a>
                @else
