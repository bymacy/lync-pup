<x-layouts.admin :title="$startup->company_name">
    ...
    <div class="rounded-xl overflow-hidden mb-6">
        <div class="bg-gradient-to-r from-rose-950 to-blue-950 text-white p-8">
            <div class="flex items-center gap-4">
                <div class="w-20 h-20 rounded-xl bg-white/10 flex items-center justify-center text-3xl font-bold">
                    {{ substr($startup->company_name, 0, 1) }}
                </div>
                <div>
                    <div class="flex items-center gap-3">
                        <h1 class="text-3xl font-bold">{{ $startup->company_name }}</h1>
                        <span class="text-xs font-medium bg-white text-gray-900 rounded-full px-3 py-1">{{ $startup->status }}</span>
                    </div>
                    <p class="text-white/80 mt-1">{{ $startup->informationSheet?->business_description }}</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 space-y-6">
            ...
            <div id="assign-coordinator" class="bg-white rounded-xl border border-gray-200 p-6">
                <div class="flex items-center justify-between">
                    <h2 class="font-bold text-gray-900">Profile Coordinator</h2>
                </div>

                @if ($startup->activeCoordinatorAssignment)
                    <div class="bg-gray-100 rounded-lg px-4 py-2 text-sm mt-3 inline-block">
